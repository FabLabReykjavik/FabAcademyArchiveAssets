#
# hello.MAX98357A.RP2040.wave.py
#    XIAO RP2040 MAX98357A I2S amp hello-world
#
# Neil Gershenfeld 11/2/24
#
...
def I2S_prog():
    ...
    set(y,15)
    label('left')
    out(pins,1).side(0b00) # output bit, clock low
    mov(x,isr) # bit delay
    label('loop1')
    jmp(x_dec,'loop1')
    mov(x,isr).side(0b01) # load bit delay, clock high
    label('loop2')
    jmp(x_dec,'loop2') # bit delay loop
    jmp(y_dec,'left') # bit loop
    #
    # shifted last bit
    #
    out(pins,1).side(0b10) # output bit, clock low
    mov(x,isr) # bit delay
    label('loop3')
    jmp(x_dec,'loop3') # bit delay loop
    mov(x,isr).side(0b11) # load bit delay clock high
    label('loop4')
    jmp(x_dec,'loop4') # bit delay loop
    #
    # right word
    #
    pull()
    ...
    mov(x,isr).side(0b11) # load bit delay, clock high
    label('loop6')
    jmp(x_dec,'loop6') # bit delay loop
    jmp(y_dec,'right') # bit loop
    #
    # shifted last bit
    #
    out(pins,1).side(0b00) # output bit, clock low
    mov(x,isr) # bit delay
    label('loop7')
    jmp(x_dec,'loop7') # bit delay loop
    mov(x,isr).side(0b01) # load bit delay, clock high
    label('loop8')
    jmp(x_dec,'loop8') # bit delay loop

class I2S_pio:
    def __init__(self,sm_id,LRC,BCLK,DIN,pio_freq,bit_delay):
        self._sm = StateMachine(sm_id,I2S_prog,freq=pio_freq,
            sideset_base=machine.Pin(BCLK),
            out_base=machine.Pin(DIN))
        self._sm.put(bit_delay)
        self._sm.exec("pull()")
...
LRC_pin = 2
DIN_pin = 4
pio_freq = 145_500_000
bit_delay = 100 # for 22.05
sample_bits = 16
word_bits = 32
I2S = I2S_pio(state_machine,LRC_pin,BCLK_pin,DIN_pin,pio_freq,bit_delay)

import math,array
loop = 100
amp = 1000
a = array.array('i',[0]*loop)
for i in range(loop):
    #x = int(amp*math.sin(math.pi*2*(i/loop)))
    x = int(amp*(1+math.sin(math.pi*2*(i/loop))/2))
    a[i] = x << 16
...
PIO0_TXF0 = PIO0_BASE + 0x010
DATA_REQUEST_INDEX = 0
size = 128
src_data = bytearray(size)
for i in range(size/8):
    src_data[8*i+0] = 0
    src_data[8*i+1] = 0
    src_data[8*i+2] = int(i)
    src_data[8*i+3] = 0
    src_data[8*i+4] = 0
    src_data[8*i+5] = 0
    src_data[8*i+6] = i
...
while True:
    d = rp2.DMA()
    c = d.pack_ctrl(size=2,
        inc_write=False,
        treq_sel=DATA_REQUEST_INDEX)
    d.config(
        read=src_data,
        write=PIO0_TXF0,
        count=int(len(src_data)/4),
        ctrl=c,
        trigger=True
        )
    d.close()    
