#!/usr/bin/env python
#
# hello.load.RP2040.ring
#    ring oscillator loading hello-world
#
# Neil Gershenfeld 7/29/24
#
...
width = 1.15 # board width
height = 0.85 # board height
x = 1 # x origin
y = 1 # y origin
zt = 0 # top z
zb = -0.06 # bottom z
...
IC1 = XIAO_RP2040('IC1\nXIAO\nRP2040')
pcb = IC1.add(pcb,x+.42,y+height-0.42)

IC2 = SN74LVC3G14DCTR('IC2\n3G14')
pcb = IC2.add(pcb,IC1.pad[12].x+.18,IC1.pad[12].y,angle=270)

pcb = wire(pcb,w,
   IC2.pad[1],
   point(IC2.pad[1].x,IC2.y+.02),
   point(IC2.pad[5].x,IC2.y+.02),
   IC2.pad[5])
...
pcb = wire(pcb,w,
   IC2.pad[2],
   point(IC2.pad[2].x,IC2.pad[2].y+.05),
   point(IC2.pad[3].x,IC2.pad[2].y+.05),
   IC2.pad[3])

pcb = wire(pcb,w,
   IC2.pad[6],
   point(IC2.pad[6].x,IC2.pad[6].y-.05),
   point(IC2.pad[7].x,IC2.pad[6].y-.05),
   IC2.pad[7])

pcb = wire(pcb,w,
   IC2.pad[7],
   point(IC2.pad[7].x,IC2.y-.02),
   point(IC2.pad[8].x+.03,IC2.y-.02),
   point(IC2.pad[8].x+.03,IC1.pad[10].y),
   IC1.pad[10])

pcb = wire(pcb,w,
   IC2.pad[8],
   point(IC2.pad[8].x,IC2.pad[8].y-.08),
   point(IC2.pad[5].x-.03,IC2.pad[8].y-.08),
   point(IC2.pad[5].x-.03,IC1.pad[12].y),
   IC1.pad[12])

C1 = C_1206('C1')
pcb = C1.add(pcb,IC2.x,IC2.y+.18)

pcb = wire(pcb,w,
   C1.pad[1],
   point(IC2.pad[4].x,C1.y),
   IC2.pad[4])

pcb = wire(pcb,w,
   C1.pad[2],
   point(IC2.pad[1].x,C1.y),
   IC2.pad[1])

J1 = hole_pad("load ",zb,zt)
pcb = J1.add(pcb,x+width-.05,IC2.y+.02)

pcb = wire(pcb,w,
   J1.pad[1],
   point(IC2.pad[1].x,J1.y),
   IC2.pad[1])

J2 = hole_pad("GND",zb,zt)
pcb = J2.add(pcb,J1.x,y+height-.05)

pcb = wire(pcb,w,
   J2.pad[1],
   point(C1.pad[1].x,J2.y),
   C1.pad[1])
