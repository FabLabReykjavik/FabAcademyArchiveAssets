...
void loop() {
   //
   // rotate servo between limits
   //
   uint32_t minimum = 48e6*0.5e-3; // 0.5 ms at 48 MHz
   uint32_t maximum = 48e6*2.5e-3; // 2.5 ms at 48 MHz
   for (uint32_t compare = minimum; compare < maximum; ++compare) {
      TCC0->CC[1].reg = compare;
      delayMicroseconds(10);
      }
   for (uint32_t compare = maximum; compare > minimum; --compare) {
      TCC0->CC[1].reg = compare;
      delayMicroseconds(10);
      }
}
