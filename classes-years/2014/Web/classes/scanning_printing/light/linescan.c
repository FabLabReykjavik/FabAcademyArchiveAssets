...
//
/*
gcc linescan.c -o linescan -I /usr/include/opencv -lm -lX11 -lopencv_core -lopencv_highgui -lopencv_imgproc
*/
...
int main(int argc, char **argv) {
   ...
   IplImage *img0g,*img1g,*diffg,*weightg;
   CvCapture* cap;
   char name[100];
   int x,width,height;
   int fd;
   ...
   */
   ...
   img0g = cvCreateImage(cvSize(IMAGE_WIDTH,IMAGE_HEIGHT),IPL_DEPTH_8U,1);
   img1g = cvCreateImage(cvSize(IMAGE_WIDTH,IMAGE_HEIGHT),IPL_DEPTH_8U,1);
   weightg = cvCreateImage(cvSize(IMAGE_WIDTH,IMAGE_HEIGHT),IPL_DEPTH_8U,1);
   diffg = cvCreateImage(cvSize(IMAGE_WIDTH,IMAGE_HEIGHT),IPL_DEPTH_8U,1);
   //
   // flush X
   //
   XFlush(D);
   ...
   img0 = cvDecodeImage(&cvmat,1);
   sprintf(name,"all_on.png");
   cvCvtColor(img0,img0g,CV_RGB2GRAY);
   cvSaveImage(name,img0g,0);
   //
   // all off
   //
   XClearWindow(D,W);
   ...
   capture_image(fd);
   cvmat = cvMat(IMAGE_HEIGHT,IMAGE_WIDTH,CV_8UC3,(void *) memory_buffer);
   img1 = cvDecodeImage(&cvmat,1);
   sprintf(name,"all_off.png");
   cvCvtColor(img1,img1g,CV_RGB2GRAY);
   cvSaveImage(name,img1g,0);
   //
   // difference
   //
   for (row = 0; row < img0g->height; ++row) {
      for (col = 0; col < img0g->width; ++col) {
         i0 = (unsigned char) ((img0g->imageData) + (img0g->widthStep)*row)[col];
         i1 = (unsigned char) ((img1g->imageData) + (img1g->widthStep)*row)[col];
         if (i0 > i1)
            ((weightg->imageData) + (weightg->widthStep)*row)[col] = i0 - i1;
         else
            ((weightg->imageData) + (weightg->widthStep)*row)[col] = 0;
         }
      }
   sprintf(name,"all_diff.png");
   cvSaveImage(name,weightg,0);
   //
   // loop over bands
   //
   nlines = 40;
   height = A.height/((float) nlines);
   for (line = 0; line < nlines; ++line) {
      ...
      cvmat = cvMat(IMAGE_HEIGHT,IMAGE_WIDTH,CV_8UC3,(void *) memory_buffer);
      img0 = cvDecodeImage(&cvmat,1);
      sprintf(name,"img_%03d_0.png",line);
      cvCvtColor(img0,img0g,CV_RGB2GRAY);
      cvSaveImage(name,img0g,0);
      //
      // background
      ...
      cvmat = cvMat(IMAGE_HEIGHT,IMAGE_WIDTH,CV_8UC3,(void *) memory_buffer);
      img0 = cvDecodeImage(&cvmat,1);
      sprintf(name,"img_%03d_1.png",line);
      cvCvtColor(img0,img0g,CV_RGB2GRAY);
      cvSaveImage(name,img0g,0);
      */
      //
      ...
      for (row = 0; row < img0->height; ++row) {
         for (col = 0; col < img0->width; ++col) {
            ...
            i1 = (unsigned char) ((img1g->imageData) + (img1g->widthStep)*row)[col];
            if (i0 > i1)
               ((diffg->imageData) + (diffg->widthStep)*row)[col] = i0 - i1;
            else
               ((diffg->imageData) + (diffg->widthStep)*row)[col] = 0;
            }
         }
      sprintf(name,"diff_%03d.png",line);
      cvSaveImage(name,diffg,0);
      }
   //
   // filter and save
   //
